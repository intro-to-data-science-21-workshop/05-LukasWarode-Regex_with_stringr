...
```{r, include=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/rstudio/hex-stickers/master/SVG/stringr.svg", dpi = 100)
```
...
2. Why `stringr` (in combination with regex)?
...
<!-- ![10%](https://i.redd.it/an2lhgcr9kc61.png) -->
<img src="https://i.redd.it/an2lhgcr9kc61.png" width="400">
...
# Meta characters in _regex_ __I__
...
# 
# 
# str_extract_all(mds_string, "(?<=[:space:])Data(?=[:space:])")
# str_extract_all(mds_string, "(?<=[:space:])Data")
# str_extract_all(mds_string, "Data(?=[:space:])")
# 
# 
# 
# 
...
# Meta characters in _regex_ __II__
...
# Meta characters in _regex_ __III__
...
# Meta characters in _regex_ __IV__
...
.pull-right-wide[
```{r}
{{science_string <- "Data Science: 8.5/10, Political Science: 9.5/10"}}
...
str_extract_all(science_string, "(Data[:space:]|Political[:space:])Science")

str_extract_all(science_string, "(Data |Political )Science")
```
...
# Meta characters in _regex_ __V__
...
background-image: url(https://raw.githubusercontent.com/rstudio/hex-stickers/master/SVG/stringr.svg)
background-size: contain
...
- `stringr` is built on the basis of of `stringi`
...
--
  ...
  - `stringi`: Comprehensive and complex framework with a variety of
...
.pull-right[
- So why not use `stringi`?
]

.pull-right[
  - Especially in combination with *regex*?
]

--

- `stringr`: Simpler, however not as flexible
]
...
.pull-right-small[
- test pull right small
]

---

.pull-left-small[
- test pull left small
]
